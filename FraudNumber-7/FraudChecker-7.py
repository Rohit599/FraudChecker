...
# Initialize logging
logging.basicConfig(filename='web_scraper-7.log', level=logging.INFO,
...
csv_file_path = 'fraud_numbers-7.csv'
progress_file_path = 'progress-7.txt'
...
def number_generator(start):
    for number in range(start, 7000000000, -1):
        yield number
...
for number in number_generator(start_number):
    while True:
        try:
            ...
            captcha_image.screenshot('captcha-7.png')

            # Use OCR to read the CAPTCHA
            captcha_text = pytesseract.image_to_string(Image.open('captcha-7.png')).strip()
