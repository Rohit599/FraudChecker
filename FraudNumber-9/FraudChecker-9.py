...
# Initialize logging
logging.basicConfig(filename='web_scraper-9.log', level=logging.INFO,
...
csv_file_path = 'fraud_numbers-9.csv'
progress_file_path = 'progress-9.txt'
...
def number_generator(start):
    for number in range(start, 9000000000, -1):
        yield number
...
for number in number_generator(start_number):
    while True:
        try:
            ...
            captcha_image.screenshot('captcha-9.png')

            # Use OCR to read the CAPTCHA
            captcha_text = pytesseract.image_to_string(Image.open('captcha-9.png')).strip()
